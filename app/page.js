...
export default function Home() {
  ...
  useEffect(() => {
    try {
      ...
      const savedColors = localStorage.getItem('vds_colors');
      const savedColorMode = localStorage.getItem('vds_colorMode');
      const savedBgMode = localStorage.getItem('vds_bgMode');
      const savedCustomBg = localStorage.getItem('vds_customBgColor');
      ...
      if (savedBgMode) setBgMode(savedBgMode);
      if (savedCustomBg) setCustomBgColor(savedCustomBg);
    } catch (e) { console.error('Error loading state from localStorage:', e); }
    
    setIsLoaded(true);
  }, []);

  // Sync selectedStyle to halftoneConfig.gridType when style changes
  useEffect(() => {
    if (selectedStyle === 'pencilmesh' || selectedStyle === 'pencilnetwork') {
      setHalftoneConfig(prev => ({ ...prev, gridType: 'network' }));
    } else if (selectedStyle === 'pencilradial') {
      setHalftoneConfig(prev => ({ ...prev, gridType: 'radial' }));
    }
  }, [selectedStyle]);

  useEffect(() => {
    if (isLoaded) {
      if (selectedStyle) localStorage.setItem('vds_selectedStyle', selectedStyle);
      localStorage.setItem('vds_colors', JSON.stringify(colors));
      localStorage.setItem('vds_colorMode', colorMode);
      localStorage.setItem('vds_bgMode', bgMode);
      localStorage.setItem('vds_customBgColor', customBgColor);
    }
  }, [selectedStyle, colors, colorMode, bgMode, customBgColor, isLoaded]);
    ...
    <main className="h-screen bg-[#050508] text-white flex overflow-hidden font-sans">
      ...
      <aside className="w-[260px] flex flex-col h-full border-r border-white/5 bg-[#0a0a0f] shrink-0 relative z-20 shadow-[10px_0_30px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4">
          ...
          <div className="mt-5 pt-4 border-t border-white/5 space-y-4">
            ...
            <ImageUploader 
              onImageLoad={(img, fileName) => { handleImageLoad(img, fileName); setAppMode('halftone'); }} 
      ...
      <aside className="w-[260px] flex flex-col h-full border-r border-white/5 bg-[#080810] shrink-0 relative z-20">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar px-4 py-4 space-y-5">
          ...
          {/* Halftone Drawing Details - Moved to Panel 2 */}
          {((selectedStyle === 'pencilnetwork' || selectedStyle === 'pencilradial' || selectedStyle === 'pencilmesh') && appMode === 'draw') || (appMode === 'halftone' && uploadedImage) ? (
          <div>
            <div className="bg-gradient-to-b from-white/[0.05] to-transparent border border-white/10 rounded-xl p-4 shadow-xl backdrop-blur-xl relative overflow-hidden group">
              ...
              <div className="space-y-4">
                ...
                <div className="pt-3 border-t border-white/5">
                  <p className="text-[11px] font-semibold text-gray-300 tracking-wide mb-2">Halftone Shape</p>
                  <select 
