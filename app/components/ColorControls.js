...
export default function ColorControls({ 
  ...
  const [useMonoBlend, setUseMonoBlend] = useState(colorMode === 'blend');
  const [isMeshSettingsOpen, setIsMeshSettingsOpen] = useState(false);

  useEffect(() => {
    setUseMonoBlend(colorMode === 'blend');
    if (colors && colors.length > 0) {
      if (colorMode === 'solid') setGlobalSolidColor(colors[0]);
      if (colorMode === 'blend') setMonoBlendColor(colors[0]);
    }
  }, [colors, colorMode]);
