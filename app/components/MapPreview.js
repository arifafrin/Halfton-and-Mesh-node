...
export default memo(function MapPreview({ 
    ...
    if (mapStyles[style]?.isPencil && (isDrawingShapeRef.current || (activeDrawTool === 'pen' && penPath.length > 0))) {
       ...
       if (activeDrawTool === 'pen') {
           ...
           if (activeDrawTool === 'line') {
               ...
               let curY = canvasY;
               if (e.shiftKey) {
                   const angle = Math.atan2(curY - origin.y, curX - origin.x);
                   const snappedAngle = Math.round(angle / (Math.PI / 4)) * (Math.PI / 4);
                   const dist = Math.sqrt(Math.pow(curX - origin.x, 2) + Math.pow(curY - origin.y, 2));
                   curX = origin.x + dist * Math.cos(snappedAngle);
                   curY = origin.y + dist * Math.sin(snappedAngle);
               }
               dString = `M ${origin.x},${origin.y} L ${curX},${curY}`;
  ...
  const styleConfig = mapStyles[style] || mapStyles.pencilmesh || {};
                ...
                <g id="halftone-overlay">
                  {(((appMode === 'draw' && !!uploadedImage) && styleConfig?.id === 'pencilmesh') || (appMode === 'halftone' && halftoneConfig.gridType === 'network')) ? (() => {
                    ...
                    {halftoneNodes.map((n, i) => {
                       ...
                       const currentShape = halftoneShape;
